...
// Operational Bounding Box: National Capital Region (NCR / Delhi)
const LAT_MIN = 28.50;
const LAT_MAX = 28.72;
const LNG_MIN = 77.10;
const LNG_MAX = 77.30;
...
const randomGeo = () => {
  const latitude = Number((LAT_MIN + Math.random() * (LAT_MAX - LAT_MIN)).toFixed(6));
  const longitude = Number((LNG_MIN + Math.random() * (LNG_MAX - LNG_MIN)).toFixed(6));
...
const SEED_LOCATIONS = [
  ...
  { name: 'Nehru Place Electronics Market Ring Road', zone: 'Zone 3 (Indust.)' },
  { name: 'Shahdara Metro Interchange Depot', zone: 'Zone 4 (East)' },
];
...
const generateSyntheticData = async () => {
  ...
  console.log('====================================================');
  console.log('⚡ EMERGENX SYNTHETIC DATA GENERATOR (PHASE 28)');
  ...
  // 2. Generate 25 Response Teams
  console.log('[2/7] Generating 25 Specialized Response Teams...');
  const teamTypes = ['FIRE', 'MEDICAL', 'POLICE', 'HAZMAT', 'RESCUE'];
  const teams = [];
  for (let i = 1; i <= 25; i++) {
    const type = teamTypes[(i - 1) % teamTypes.length];
    const geo = randomGeo();
    const loc = SEED_LOCATIONS[i % SEED_LOCATIONS.length];
    const teamId = `TEAM-SYN-${String(i).padStart(2, '0')}`;

    teams.push({
      teamId,
      name: `${loc.zone} ${type} Unit ${i}`,
      type,
      status: i % 4 === 0 ? 'ASSIGNED' : i % 7 === 0 ? 'BUSY' : 'AVAILABLE',
      members: [`Lead Officer ${i}`, `Technician ${i}-A`, `Medic ${i}-B`, `Specialist ${i}-C`],
      capabilities: [`${type}_RESPONSE`, 'RAPID_DEPLOYMENT'],
      location: {
        latitude: geo.latitude,
        longitude: geo.longitude,
        address: `${loc.name} Staging Post`,
  ...
  // 3. Generate 60 Resource Apparatus
  console.log('[3/7] Generating 60 Heavy & Tactical Resource Apparatus...');
  const resTypes = ['FIRE_VEHICLE', 'AMBULANCE', 'POLICE_VEHICLE', 'HAZMAT_UNIT', 'RESCUE_EQUIPMENT', 'AIRCRAFT_DRONE'];
  const resources = [];
  for (let i = 1; i <= 60; i++) {
    const type = resTypes[(i - 1) % resTypes.length];
    const geo = randomGeo();
    const loc = SEED_LOCATIONS[i % SEED_LOCATIONS.length];
    const resourceId = `RES-SYN-${String(i).padStart(3, '0')}`;
    const assignedTeamId = teams[i % teams.length].teamId;

    resources.push({
      resourceId,
      name: `${type.replace('_', ' ')} #${i}`,
      type,
      status: i % 5 === 0 ? 'ASSIGNED' : i % 8 === 0 ? 'EN_ROUTE' : 'AVAILABLE',
      location: {
        latitude: geo.latitude,
        longitude: geo.longitude,
        address: loc.name,
      ...
      capabilities: ['RAPID_RESPONSE', 'HEAVY_DUTY'],
      capacity: 2 + (i % 8),
      metadata: { isSynthetic: true, assignedTeamId },
    });
  }
  await ResourceModel.insertMany(resources);
  console.log(`✓ Seeded ${resources.length} Operational Resources.`);

  // 4. Generate 15 Hospitals & Emergency Facilities
  console.log('[4/7] Generating 15 Trauma Centers and Hospital Facilities...');
  const facilities = [];
  const hospitalNames = [
    'All India Institute of Medical Sciences (Trauma Center)',
    'Safdarjung Super-Specialty Emergency Hospital',
    'Max Super Specialty Burn & Trauma Pavilion',
    'Apollo Regional Disaster Response Trauma ICU',
    'Fortis Escorts Emergency Heart & Trauma Center',
    'Sir Ganga Ram Critical Casualty Facility',
    'Ram Manohar Lohia Emergency Command Hospital',
    'Lok Nayak Multi-Specialty Acute Ward',
    'Guru Teg Bahadur Regional Trauma Pavilion',
    'Indraprastha Medical Complex Emergency ICU',
    'Moolchand Disaster Support Pavilion',
    'Holy Family Acute Critical Care Center',
    'Bara Hindu Rao Emergency Casualty Ward',
    'Deen Dayal Upadhyay Regional Trauma Center',
    'Sanjay Gandhi Memorial Emergency Staging Post',
  ];

  for (let i = 0; i < hospitalNames.length; i++) {
    const geo = randomGeo();
    const totalBeds = 120 + Math.floor(Math.random() * 200);
    const occupiedBeds = Math.floor(totalBeds * (0.65 + Math.random() * 0.3));

    facilities.push({
      facilityId: `FAC-SYN-${String(i + 1).padStart(2, '0')}`,
      name: hospitalNames[i],
      type: 'HOSPITAL',
      location: {
        latitude: geo.latitude,
        longitude: geo.longitude,
        address: hospitalNames[i] + ', NCR',
        geometry: { type: 'Point', coordinates: [geo.longitude, geo.latitude] },
      },
      capacity: totalBeds,
      availableCapacity: totalBeds - occupiedBeds,
      status: 'OPERATIONAL',
      contactNumber: `+91-11-2338${String(1000 + i).slice(-4)}`,
      metadata: { isSynthetic: true },
    });
  }
  await FacilityModel.insertMany(facilities);
  console.log(`✓ Seeded ${facilities.length} Emergency Facilities.`);
  ...
  const statuses = ['NEW', 'ACKNOWLEDGED', 'ASSIGNED', 'RESPONDING', 'ON_SCENE', 'RESOLVED'];
  ...
  for (let i = 1; i <= targetIncidentCount; i++) {
    ...
    const assignedRes = resources[i % resources.length];
    ...
    const incDoc = {
      ...
      title,
      type: cat,
      description: `Automated telemetry & citizen reports describing ${title.toLowerCase()} in ${loc.name}.`,
      location: {
        latitude: geo.latitude,
        longitude: geo.longitude,
        address: `${loc.name}, ${loc.zone}`,
      ...
      reports: [
        {
          ...
          text: `Distress caller reports rapid escalation at ${loc.name}.`,
          reliability: 92,
          reportedAt,
        },
        {
