    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          {/* AI Incident Classification & Operational Triage Card */}
          <div className="p-6 rounded-[18px] bg-white dark:bg-[rgba(11,14,19,0.85)] border border-purple-500/30 dark:border-[rgba(124,92,252,0.4)] backdrop-blur-[20px] relative overflow-hidden shadow-lg dark:shadow-[0_12px_45px_rgba(0,0,0,0.5)]">
            ...
            {/* Header: Title, Engine Tag, and Run / Re-Analyze Trigger */}
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="text-sm font-display font-bold text-slate-900 dark:text-white">
                      AI INCIDENT CLASSIFICATION & TRIAGE
                    </h3>
                    <span className="px-1.5 py-0.5 rounded text-[9px] font-mono font-bold uppercase bg-purple-100 dark:bg-purple-950/60 text-purple-700 dark:text-purple-300 border border-purple-300 dark:border-purple-800/60">
                      {incident.aiAnalysis?.model || 'emergency-classifier-v1'}
                    </span>
                  </div>
                  <p className="text-[10px] font-mono text-slate-500 dark:text-slate-400">
                    HYBRID SEMANTIC NLP • DETERMINISTIC SAFETY OVERRIDES • CALIBRATED SCORING
                  </p>
                </div>
              </div>

              {/* Status Indicator & Re-Analyze Action */}
              <div className="flex items-center gap-2.5">
                {/* AI Status Badge */}
                {(!incident.aiAnalysis || incident.aiAnalysis.status === 'COMPLETED') && (
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-mono font-bold bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-400 border border-emerald-300 dark:border-emerald-800/60">
                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                    AI VERIFIED
                  </span>
                )}
                {incident.aiAnalysis?.status === 'PROCESSING' && (
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-mono font-bold bg-cyan-50 dark:bg-cyan-950/40 text-cyan-700 dark:text-cyan-400 border border-cyan-300 dark:border-cyan-800/60">
                    <RefreshCw className="w-3 h-3 animate-spin text-cyan-500" />
                    ANALYZING...
                  </span>
                )}
                {incident.aiAnalysis?.status === 'FAILED' && (
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-mono font-bold bg-rose-50 dark:bg-rose-950/40 text-rose-700 dark:text-rose-400 border border-rose-300 dark:border-rose-800/60">
                    <AlertCircle className="w-3 h-3 text-rose-500" />
                    AI FAILED
                  </span>
                )}
                {incident.aiAnalysis?.status === 'PENDING' && (
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-mono font-bold bg-amber-50 dark:bg-amber-950/40 text-amber-700 dark:text-amber-400 border border-amber-300 dark:border-amber-800/60">
                ...
                <button
                  ...
                  disabled={isAnalyzingAi || incident.aiAnalysis?.status === 'PROCESSING'}
            ...
            {/* Error Feedback Banner if AI failed */}
            {aiError && (
              <div className="mb-4 p-3 rounded-xl bg-rose-50 dark:bg-rose-950/40 border border-rose-300 dark:border-rose-800/60 text-rose-700 dark:text-rose-300 text-xs font-mono flex items-center gap-2">
                <AlertCircle className="w-4 h-4 flex-shrink-0" />
                <span>{aiError}</span>
              </div>
            )}

            {/* Type Correction Callout Banner */}
            {incident.aiAnalysis?.suggestedCorrection && (
              <div className="mb-4 p-3.5 rounded-xl bg-indigo-50 dark:bg-indigo-950/40 border border-indigo-200 dark:border-indigo-800/60 text-indigo-900 dark:text-indigo-200 text-xs font-sans flex items-start gap-2.5">
                <Sparkles className="w-4 h-4 text-indigo-500 flex-shrink-0 mt-0.5" />
                <div>
                  <strong className="font-semibold block font-mono text-[11px] uppercase tracking-wider text-indigo-700 dark:text-indigo-300">
                    AI Category Correction Applied
                  </strong>
                  <span>
                    Reporting intake flagged this incident as <strong className="font-mono underline">{incident.aiAnalysis.originalType || incident.rawType || 'OTHER'}</strong>, but AI signal evidence corrected classification to <strong className="font-mono text-purple-700 dark:text-purple-300">{incident.aiAnalysis.incidentType}</strong>.
                  </span>
                </div>
              </div>
            )}
                  ...
                  <div className="flex-1 h-2 bg-slate-200 dark:bg-white/10 rounded-full overflow-hidden">
                    <div
                      className={`h-full rounded-full ${
                        (incident.aiAnalysis?.confidence ?? 0.8) >= 0.80
                          ? 'bg-emerald-500'
                          : (incident.aiAnalysis?.confidence ?? 0.8) >= 0.70
                          ? 'bg-cyan-500'
                          : 'bg-amber-500'
                      }`}
                      style={{
            ...
            {/* Extracted Key Tactical Signals */}
            {incident.aiAnalysis?.signals && incident.aiAnalysis.signals.length > 0 && (
              <div className="mb-4">
                <span className="text-[10px] font-mono font-bold text-slate-500 dark:text-slate-400 block mb-2 uppercase tracking-wider">
                  DETECTED TACTICAL SIGNALS ({incident.aiAnalysis.signals.length}):
                </span>
                <div className="flex flex-wrap gap-1.5">
                  {incident.aiAnalysis.signals.map((sig, idx) => (
                    <span
                      key={idx}
                      className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-[11px] font-mono font-semibold bg-purple-50 dark:bg-purple-950/40 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-800/50"
                    >
            ...
            {/* Machine-Readable Reasoning Breakdown */}
            {incident.aiAnalysis?.reasoning && (
              <div className="p-4 rounded-xl bg-slate-50 dark:bg-white/[0.02] border border-slate-200 dark:border-white/5 space-y-2.5 text-xs">
                <span className="text-[10px] font-mono font-bold text-purple-700 dark:text-[#A78BFA] block uppercase tracking-wider">
                  EXPLAINABLE AI REASONING & DECISION SUPPORT:
                </span>
                {incident.aiAnalysis.reasoning.incidentType && (
                  <div>
                    <strong className="text-slate-700 dark:text-slate-300 font-semibold font-mono text-[11px] block">
                      • Category Reasoning:
                    </strong>
                    <p className="text-slate-600 dark:text-slate-400 font-sans pl-2.5">
                      {incident.aiAnalysis.reasoning.incidentType}
                    </p>
                  </div>
                )}
                {incident.aiAnalysis.reasoning.severity && (
                  <div>
                    <strong className="text-slate-700 dark:text-slate-300 font-semibold font-mono text-[11px] block">
                      • Severity Evidence:
                    </strong>
                ...
                {incident.aiAnalysis.reasoning.priority && (
                  <div>
                    <strong className="text-slate-700 dark:text-slate-300 font-semibold font-mono text-[11px] block">
            ...
            {incident.metadata && Object.keys(incident.metadata).length > 0 && (
              <div className="mt-4 pt-3 border-t border-slate-200 dark:border-white/10 font-mono text-xs">
                <span className="text-[10px] text-slate-500 block mb-1.5 font-bold uppercase">Source Intake Metadata:</span>
