...
export const MetricCards: React.FC = () => {
  ...
  const metrics = [
    ...
    {
      title: 'ESCALATIONS',
      value: String(stats.activeEscalations ?? 0).padStart(2, '0'),
      change: 'Level 1/2/3 triggers',
      trend: (stats.activeEscalations ?? 0) > 0 ? 'critical' : 'neutral',
      icon: AlertOctagon,
      color: 'text-[#F5A623]',
      variant: (stats.activeEscalations ?? 0) > 0 ? ('warning' as const) : ('default' as const),
      route: '/alerts',
    },
    {
    ...
    {
      title: 'UNREAD ALERTS',
      value: String(stats.unreadNotifications ?? 0).padStart(2, '0'),
      change: 'Live broadcast queue',
      trend: (stats.unreadNotifications ?? 0) > 0 ? 'warning' : 'neutral',
      icon: Bell,
      color: 'text-[#A78BFA]',
      variant: 'ai' as const,
      route: '/notifications',
